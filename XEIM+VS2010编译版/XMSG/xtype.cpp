...
XUserDetail::XUserDetail(const XUserDetail &xUserDetail)
{
	...
	strcpy_s(m_szUPassword, 32*sizeof(char),xUserDetail.m_szUPassword);
	strcpy_s(m_szUID, 32*sizeof(char),xUserDetail.m_szUID);
	strcpy_s(m_szUNickName, 32*sizeof(char),xUserDetail.m_szUNickName);
	strcpy_s(m_szUGroup, 32*sizeof(char),xUserDetail.m_szUGroup);
...
{
	strcpy_s(m_szUID,32*sizeof(char), szUID);
}

// 用户昵称
void XUserDetail::SetUNickName(const char *szUNickName)
{
	strcpy_s(m_szUNickName, 32*sizeof(char), szUNickName);
}

// 用户密码
void XUserDetail::SetUPassword(const char *szUPassword)
{
	strcpy_s(m_szUPassword,32*sizeof(char), szUPassword);
}

// 用户组信息
void XUserDetail::SetUGroup(const char *szUGroup)
{
	strcpy_s(m_szUGroup,32*sizeof(char), szUGroup);
}
...
{
	int ret = strcmp(_strlwr(m_szUID), strlwr((char*)xUserDetail.m_szUID));
	if (ret != 0)
		return FALSE;
